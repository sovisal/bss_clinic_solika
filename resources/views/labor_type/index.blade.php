<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-bottom">
            <div>
                @if (request()->type)
                <x-form.button color="danger" href="{!! route('setting.labor-type.index', ['type' => request()->old]) !!}" label="Back"
                    icon="bx bx-left-arrow-alt" />
                @endif
                <x-form.button color="dark" href="{!! route('setting.labor-type.sort_order', request()->only(['type', 'old'])) !!}" label="Sort Type"
                    icon="bx bx-sort-alt-2" />
                <x-form.button color="dark" href="{!! route('setting.labor-item.sort_order', request()->only(['type', 'old'])) !!}" label="Sort Item"
                    icon="bx bx-sort-alt-2" />
            </div>
        ...
        <x-table class="table-hover table-striped" id="datatables-labor-type">
            ...
            <tr>
                ...
                <td class="text-center">
                    <x-form.button class="btn-sm" href="{!! route('setting.labor-type.index', ['type' => $row->id, 'old' => request()->type]) !!}"
                        icon="bx bx-detail" />
                    <x-form.button color="secondary" class="btn-sm"
                        href="{!! route('setting.labor-type.edit', ['laborType' => $row->id, 'type' => request()->type]) !!}" icon="bx bx-edit-alt" />
                    <x-form.button color="danger" class="confirmDelete btn-sm" data-id="{{ $row->id }}" icon="bx bx-trash"
                        :disabled="(count($row->items) > 0 || count($row->types))" />
                    <form class="sr-only" id="form-delete-{{ $row->id }}"
                        action="{!! route('setting.labor-type.delete', ['laborType' => $row->id, 'type' => request()->type]) !!}" method="POST">
                        @csrf
                        @method('DELETE')
    ...
    <x-card :foot="false">
        ...
        <x-table class="table-hover table-striped" id="datatables-labor-item">
            ...
            <tr>
                ...
                <td class="text-center">
                    <x-form.button color="secondary" class="btn-sm"
                        href="{!! route('setting.labor-item.edit', ['laborItem' => $item_row->id, 'type' => request()->type, 'old' => request()->old]) !!}"
                        icon="bx bx-edit-alt" />
                    <x-form.button color="danger" class="confirmDelete btn-sm" data-id="{{ $item_row->id }}" icon="bx bx-trash" />
                    <form class="sr-only" id="form-delete-{{ $item_row->id }}"
                        action="{!! route('setting.labor-item.delete', ['laborItem' => $item_row->id, 'type' => request()->type, 'old' => request()->old]) !!}"
                        method="POST">
                        @csrf
                        @method('DELETE')
