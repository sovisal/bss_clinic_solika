<x-app-layout>
    <x-slot name="header">
        <x-form.button-back href="{{ route('user.role.index') }}" />
    </x-slot>
                    ...
                    <td style="vertical-align: top;">
                        <ul class="list-group">
                            <li class="list-group-item border-0 d-flex justify-content-between">
                                <x-form.checkbox name="ability_module[]" class="chb_all" id="chb_all_{{ $ability_module->id }}"
                    ...
                    <td>
                        <ul class="list-group list-group-child">
                            ...
                            <li class="list-group-item border-0 tw-py-2">
                                <x-form.checkbox name="ability[]" class="chb_child" id="chb_{{ $ability->id }}" value="{{ $ability->name }}"
                                    checked="{{ $role->abilities()->contains($ability->name) }}" label="{{ $ability->name }}" />
                            </li>
                            @endif
                            @endforeach
                        </ul>
