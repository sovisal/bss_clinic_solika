<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-bottom">
            <div>
                <x-form.button color="danger" href="{!! route('setting.labor-type.index', ['type' => request()->type, 'old' => request()->old]) !!}"
    ...
    <form action="{{ route('setting.labor-item.update_order', ['type' => request()->type, 'old' => request()->old]) }}" method="POST">
        @csrf
            ...
            <x-dragable-list>
                ...
                <x-dragable-item>
                    <x-slot name="rightSide">
                        {{ $row->hasType->name_kh }} : <strong>TYPE</strong>
                    </x-slot>
